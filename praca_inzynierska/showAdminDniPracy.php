...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_array($result)) {
			...
			$pocz = get_godzina_by_id($row["dzien_id_godzina_pocz"]);
			$item["poczatek"] = $pocz["godzina"];
			$kon = get_godzina_by_id($row["dzien_id_godzina_kon"]);
			$item["koniec"] = $kon["godzina"];
			...
			$date3 = strtotime(date('Y-m-d'). ' + 30 days');
...
function get_godzina_by_id($id){
	global $connect;
		//godzina po id
		
		$query = mysqli_query($connect,'SELECT * FROM godzina WHERE id_godzina='.$id) or die(mysql_error());
